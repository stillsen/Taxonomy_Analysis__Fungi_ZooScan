...
df %>%
  ...
  arrange(Rank)%>%
ggplot()+
  geom_point(aes_string(y="Score", x="Rank", colour="Model", shape="Dataset", group="grp"), size = 3.5, position =position_dodge(width = .25))+
  geom_line(aes_string(y="Score", x="Rank", colour="Model", group="grp"), position =position_dodge(width = .25))
