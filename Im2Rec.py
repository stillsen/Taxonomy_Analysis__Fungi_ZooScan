# Wrapper class for im2rec by Apache Software Foundation (ASF) to inhance usage
# -*- coding: utf-8 -*-
...
class Im2Rec:
            ...
            for fname in files:
                if fname.startswith(args.prefix) and fname.endswith('.lst'):
                    ...
                    if args.num_thread > 1 and multiprocessing is not None:
                        ...
                        # define the process
                        read_process = [multiprocessing.Process(target=self.read_worker, args=(args, q_in[i], q_out)) \
    ...
    def list_image(self, root, recursive, exts):
    ...
    def write_list(self, path_out, image_list):
    ...
    def make_list(self, args):
        ...
        """
        image_list = self.list_image(args.root, args.recursive, args.exts)
        ...
        for i in range(args.chunks):
            ...
            if args.train_ratio == 1.0:
                self.write_list(args.prefix + str_chunk + '.lst', chunk)
            else:
                if args.test_ratio:
                    self.write_list(args.prefix + str_chunk + '_test.lst', chunk[:sep_test])
                if args.train_ratio + args.test_ratio < 1.0:
                    self.write_list(args.prefix + str_chunk + '_val.lst', chunk[sep_test + sep:])
                self.write_list(args.prefix + str_chunk + '_train.lst', chunk[sep_test:sep_test + sep])
    ...
    def read_worker(self, args, q_in, q_out):
    ...
    def write_worker(self, q_out, fname, working_dir):
